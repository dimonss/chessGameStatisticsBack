...
```bash
# Только миграции (создание таблиц)
npm run init-db

# Миграции + заполнение тестовыми данными
...
Для инициализации базы данных доступны следующие команды:

- `npm run init-db` - Создает таблицы и индексы (только миграции)
- `npm run init-db:seed` - Создает таблицы и заполняет тестовыми данными
- `npm run seed` - Заполняет базу данных тестовыми данными (требует, чтобы таблицы уже существовали)

**Рекомендуется использовать `npm run init-db:seed` для первого запуска проекта.**
